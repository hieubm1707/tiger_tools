...
class ServiceConfigCommand extends Command {
  @override
  String get description => 'Generate a service config file.';
...
void runServiceConfig() {
  ...
  if (selectedServicesIndexes.isEmpty) {
    stderr.writeln('No service selected, use \'space\' to select a service');
    return;
  }
  ...
  if (selectedServicesIndexes.length == 1) {
    ...
    for (final serviceIndex in selectedServicesIndexes) {
      ...
      if (!inputFile.existsSync() || !outFile.existsSync()) {
        stdout.writeln('$inputPath or $outputDirectoryPath was not found');
        return;
      }
